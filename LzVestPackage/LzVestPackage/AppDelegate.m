...
- (void)compareSwitchStatus {
    [ZWBmobManager compareSwitchStatus:^(NSInteger switch_, NSURL *link, NSString *appid) {
        if (switch_ && [appid isEqualToString:APPID]) {
            //open
...
- (void)loadWebViewWith:(NSURL *)url {
    //将得到的URL存储下来
    NSString * resultStr = url.absoluteString;
    [ZWVestManager saveObj:resultStr WithKey:linkURLKey];
